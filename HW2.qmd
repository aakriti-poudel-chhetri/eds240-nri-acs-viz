...
```{r}
#| fig-height: 12
#| fig-alt: A lollipop chart showing median National Risk Index scores for 50 US states arranged from highest to lowest risk. California, Delaware and New Jersey have the highest risk scores. States like North Dakota, South Dakota and Kansas have the lowest risk scores.
...
ggplot(california, aes(x = reorder(state_name, median_risk_score), 
  ...
  labs(x = NULL, 
       y = "Median risk score", 
       title = "FEMA’s National Risk Index (NRI) Score for Natural Hazards in US States",
       subtitle = "The median NRI score for US states",
  ...
  theme(legend.position = "bottom",
        ...
        plot.caption = element_text(hjust = 0, color = "black", size = 20 ),
...
## Identify variables of interest and consider which data types they are
...
**Answer:** I chose a lollipop chart because it shows rankings for all 50 states in a clear, uncluttered way. The horizontal layout also makes the state names easy to read. Other options could be a bar chart, but a bar chart may feel too heavy with 50 states. I settled on the lollipop chart because it keeps the graphic clean, makes ranking easy to see, and helps California stand out relative to other states.
...
I made several changes to improve readability. The states were ordered from highest to lowest median risk score so comparisons are easy to follow, and California was highlighted in a different color to make it stand out. The chart was flipped so state names are easier to read, the figure height and text sizes were increased to reduce crowding, and a clean theme was used. I also added a clear title, subtitle, caption with the data source, and alt text to improve accessibility.

5. Is there anything you wanted to implement, but didn’t know how? If so, please describe.

**Answer:** I considered adding a vertical reference line to show the national median risk score, but I wasn’t sure how to implement it in the plot. I also thought about using state abbreviations instead of full names to save space, but didn’t know how to join the abbreviation data correctly.
